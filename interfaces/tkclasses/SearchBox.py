...
class SearchBox(tk.Entry):
    ...
    def set_listbox_visible(self):
        ...
        wl, hl, xl ,yl = self.listbox.winfo_geometry().replace('x','+').split('+')
        w, h, x ,y = self.winfo_geometry().replace('x','+').split('+')
    ...
    def __key_pressed(self, event):
        ...
        if event.char == enter and not self.command == None:
            ...
            if not self.listbox_is_visible:
                self.set_listbox_visible()
            lista_listbox = list(self.listbox.get(0,tk.END))
            lista_para_adicionar = list(filter(lambda x: entrada in x.lower(), self.lista))

            self.listbox.delete(0, tk.END)

            for x in lista_para_adicionar:
                if not x in list(self.listbox.get(0,tk.END)):
                    self.listbox.insert(tk.END, x)